...
class bd{
	...
	public function insertRow ($data,$sw=1,$swadicional=1){
		...
		if($swadicional==1){
			...
			$fecha=date("Y-m-d");
			$hora=date("H:i:s");
			array_push($key,"FechaRegistro");
			array_push($key,"HoraRegistro");
...
//		return $this->validateOperation ();
	...
	function mostrar($Cod){
		$this->campos=array("*");
		return $this->getRecords("Cod".ucwords($this->tabla)."=$Cod");
	}
	
	function mostrarTodo($where='',$orden=false,$cantidad=0){
		$this->campos=array('*');
		$orden=$orden?$orden:"Cod".ucwords($this->tabla);
...
//		echo "asd".$orden;
	...
	function actualizar($values,$Cod){
		$this->updateRow($values,"Cod".ucwords($this->tabla)."=$Cod");	
	}
	function eliminar($Cod){
		$this->updateRow(array("Activo"=>0),"Cod".ucwords($this->tabla)."=$Cod");	
	}
	/*Fin de Metodos*/
}
?>